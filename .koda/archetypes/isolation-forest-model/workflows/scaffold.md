---
description: Scaffold governed Isolation Forest Model training pipeline with MLflow tracking and compliance (Isolation Forest Model)
---
...
- Load `${ARCHETYPES_BASEDIR}/isolation-forest-model/templates/env-config.yaml` for configuration
...
## Examples
**Example 1**: `/scaffold-isolation-forest-model Create training pipeline with governance` - Output: Complete ML pipeline with MLflow
**Example 2**: `/scaffold-isolation-forest-model Generate model with fairness checks` - Output: Governed pipeline with fairness validation
