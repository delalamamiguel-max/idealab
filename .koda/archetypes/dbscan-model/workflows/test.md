---
description: Validate Dbscan Model pipeline for quality, fairness, and promotion readiness (Dbscan Model)
---
...
### 1. Environment Setup
Run `python ${ARCHETYPES_BASEDIR}/00-core-orchestration/scripts/validate_env.py --archetype dbscan-model --json ` and parse for MLFLOW_TRACKING_URI, PYTEST_VERSION, ENV_VALID. Halt if ENV_VALID is false.

### 2. Load Configuration
- The constitution rules are already loaded in context above.
- Load `${ARCHETYPES_BASEDIR}/dbscan-model/templates/env-config.yaml` for test specifications
...
## Examples
**Example 1**: `/test-dbscan-model Validate ML pipeline` - Output: Test suite with validation
**Example 2**: `/test-dbscan-model Fairness testing` - Output: Fairness validation suite
